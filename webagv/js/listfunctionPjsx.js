...
function showList(className, currpage, url_para) {
    ...
    if (rows.length == 0) {
        ...
        for (i = 0; i < rows.length; i ++) {
            ...
            str = str + getRow("<img class=\"button\" src=\""+GBasePath+"/images/default/edit.gif\" onclick=\"javascript:window.location='" + className + "Action.jsp?cmd=JPjsx&Id=" + rows[i][keyindex] + "&page=" + currpage + ((url_para.length == 0) ? "" : "&" + url_para) + "'\">", color);
            str = str + "</tr>\n";
        }
    }
    str = str + '</table>';
