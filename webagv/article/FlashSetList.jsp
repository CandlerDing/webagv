...
function save() {
            if (doValidate('postForm_FlashSet')) postForm_FlashSet.submit();
}
function initP() {
...
<div id="nav1" style="display:">
    <ul>
        ...
        <li><a href="javascript:document.queryForm_FlashSet.cmd.value = 'list' ;document.queryForm_FlashSet.submit();"><img src="<%=request.getContextPath()%>/images/default/button_cx.gif" alt="查询"></a></li>
        <li><a href="javascript:document.queryForm_FlashSet.cmd.value = 'excel'; document.queryForm_FlashSet.submit();"><img src="<%=request.getContextPath()%>/images/default/button_dc.gif" alt="导出Excel文件"></a></li>
        <li><b>[FLASH设置]</b></li>
...
<form action="<%=request.getAttribute("classname")%>Action.jsp" name="queryForm_FlashSet" id="queryForm_FlashSet">
<input type="hidden" name="cmd" value="list">
<%=Tool.join("\n", forms)%>
<tr>
<input type="hidden" name="_Xh_" value="<%=ParamUtils.getParameter(request, "_Xh_", "")%>">
</tr><tr>
<td>
<div align="right">图片路径</div>
</td><td>
<input name="_ImgUrl_" value="<%=ParamUtils.getParameter(request, "_ImgUrl_", "")%>">
</td>
<td>
...
<input type="button" value="Reset" onClick="clearForm('queryForm_FlashSet')" />
</td></tr></form></table>
</div>
