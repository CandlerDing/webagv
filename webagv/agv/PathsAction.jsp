...
private static final String[] allFormNames = {"cmd", "page", "idlist", "Id", "Code", "Name", "IdP", "Start", "End", "Len", "Dire", "Type", "FxType", "Isuse", "Isct", "Isbr", "Braddress", "Orderno", "Remark"};
private String[] DicKeys = {"Id", "Code", "Name", "IdP", "Start", "End", "Len", "Dire", "Type", "FxType", "Isuse", "Isct", "Isbr", "Braddress", "Orderno", "Remark"};
private String[][] DicValues = {{"int", "id", "-1", "hidden", ""}, {"string", "编码", "50", "text", ""}, {"string", "名称", "50", "text", ""}, {"int", "父ID", "-1", "hidden", ""}, {"string", "起点", "50", "text", ""}, {"string", "终点", "50", "text", ""}, {"string", "长度", "50", "text", ""}, {"string", "方向", "50", "text", ""}, {"string", "类型", "50", "text", ""}, {"string", "方向类型", "50", "text", ""}, {"int", "是否可用", "-1", "YesNo", ""}, {"int", "是否有磁条", "-1", "YesNo", ""}, {"int", "是否有避让区", "-1", "YesNo", ""}, {"string", "避让区位置", "50", "text", ""}, {"int", "排序", "-1", "text", ""}, {"string", "备注", "50", "text", ""}};
private String KeyField = "Id";
private String OperateField = DicKeys[DicKeys.length-1];//操作字段
private String[] AllFields = {"Id", "Code", "Name", "IdP", "Start", "End", "Len", "Dire", "Type", "FxType", "Isuse", "Isct", "Isbr", "Braddress", "Orderno", "Remark"};
private String[] ListFields = {"Code", "Name", "Start", "End", "Len", "Dire", "Type", "FxType", "Isuse", "Isct", "Isbr", "Braddress", "Orderno", "Remark"};
private String[] PrintFields = {"Code", "Name", "Start", "End", "Len", "Dire", "Type", "FxType", "Isuse", "Isct", "Isbr", "Braddress", "Orderno", "Remark"};
private String[] FormFields = {"Code", "Name", "Start", "End", "Len", "Dire", "Type", "FxType", "Isuse", "Isct", "Isbr", "Braddress", "Orderno", "Remark"};
private String[] QueryFields = {"Name", "Start", "End", "Len", "Dire", "Type", "FxType"};
private Paths getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("update") || cmd.equals("jqueryUpdate") ) {
        ...
        v.paramStart(request, "Start");
        v.paramEnd(request, "End");
        v.paramLen(request, "Len");
        v.paramDire(request, "Dire");
        v.paramType(request, "Type");
        v.paramFxType(request, "FxType");
        v.paramIsuse(request, "Isuse");
        v.paramIsct(request, "Isct");
        v.paramIsbr(request, "Isbr");
        v.paramBraddress(request, "Braddress");
        ...
        v.setStart(ParamUtils.getParameter(request, "Start", ""));
        v.setEnd(ParamUtils.getParameter(request, "End", ""));
        v.setLen(ParamUtils.getParameter(request, "Len", ""));
        v.setDire(ParamUtils.getParameter(request, "Dire", ""));
        v.setType(ParamUtils.getParameter(request, "Type", ""));
        v.setFxType(ParamUtils.getParameter(request, "FxType", ""));
        v.setIsuse(ParamUtils.getIntParameter(request, "Isuse", -1));
        v.setIsct(ParamUtils.getIntParameter(request, "Isct", -1));
        v.setIsbr(ParamUtils.getIntParameter(request, "Isbr", -1));
        v.setBraddress(ParamUtils.getParameter(request, "Braddress", ""));
...
private List getListRows(javax.servlet.http.HttpServletRequest request, Paths pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        Paths v = (Paths)it.next();
        List row = new ArrayList();
        row.add("" + v.getId());
        row.add(Tool.jsSpecialChars(v.getCode()));
        row.add(Tool.jsSpecialChars(v.getName()));
        row.add("" + v.getIdP());
        row.add(Tool.jsSpecialChars(v.getStart()));
        row.add(Tool.jsSpecialChars(v.getEnd()));
        row.add(Tool.jsSpecialChars(v.getLen()));
        row.add(Tool.jsSpecialChars(v.getDire()));
        row.add(Tool.jsSpecialChars(v.getType()));
        row.add(Tool.jsSpecialChars(v.getFxType()));
        row.add((String)YesNomap.get("" + v.getIsuse()));
        row.add((String)YesNomap.get("" + v.getIsct()));
        row.add((String)YesNomap.get("" + v.getIsbr()));
        row.add(Tool.jsSpecialChars(v.getBraddress()));
...
private void setListData(javax.servlet.http.HttpServletRequest request, Paths pv, List cdt)
{
    List rows = new ArrayList();
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, Paths pv, boolean isAll)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    ...
    qval = ParamUtils.getParameter(request, "_Start_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("Start like '%" + qval + "%'");
    }
    qval = ParamUtils.getParameter(request, "_End_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("End like '%" + qval + "%'");
    }
    qval = ParamUtils.getParameter(request, "_Len_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("Len like '%" + qval + "%'");
    }
    qval = ParamUtils.getParameter(request, "_Dire_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("Dire like '%" + qval + "%'");
    }
    ...
    qval = ParamUtils.getParameter(request, "_FxType_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("FxType like '%" + qval + "%'");
    }
    if (!isAll) {
...
private void setList(javax.servlet.http.HttpServletRequest request, boolean isAll)
{
    ...
    request.setAttribute("classname", "Paths");
    request.setAttribute("describe", "路径管理");
    request.setAttribute("OperateField",OperateField);
}
private List getListJsonRows(javax.servlet.http.HttpServletRequest request, Paths pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    Map YesNomap = CEditConst.getYesNoMap(userInfo);
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        map.put("Start",Tool.jsSpecialChars(v.getStart()));
        map.put("End",Tool.jsSpecialChars(v.getEnd()));
        map.put("Len",Tool.jsSpecialChars(v.getLen()));
        map.put("Dire",Tool.jsSpecialChars(v.getDire()));
        map.put("Type",Tool.jsSpecialChars(v.getType()));
        map.put("FxType",Tool.jsSpecialChars(v.getFxType()));
        map.put("Isuse",(String)YesNomap.get("" + v.getIsuse()));
        map.put("Isct",(String)YesNomap.get("" + v.getIsct()));
        map.put("Isbr",(String)YesNomap.get("" + v.getIsbr()));
        map.put("Braddress",Tool.jsSpecialChars(v.getBraddress()));
...
private void setListJsonData(javax.servlet.http.HttpServletRequest request, Paths pv, List cdt)
{
...
private void setForm(javax.servlet.http.HttpServletRequest request, Paths form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
 ...
 */
private boolean isDuplicated(Paths form, String cmd)
{
    List cdt = new ArrayList();
...
private List mkRtn(String cmd, String forward, Paths form)
{
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    String cmd = ParamUtils.getParameter(request, "cmd", "list");
    log.debug("cmd=" + cmd);
    Paths form = getByParameterDb(request);
    if (cmd.equals("list"))
    {
        setList(request);
...
forwardMap.put("list", "PathsAction.jsp");
forwardMap.put("failure", "PathsForm.jsp");
forwardMap.put("success", "PathsList.jsp");
forwardMap.put("listjquery", "PathsListJquery.jsp");
if(userInfo!=null)
{
...
forwardMap.put("input", "PathsForm.jsp");
forwardMap.put("jqueryInput", "PathsFormJquery.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script type=\"text/JavaScript\">window.location='PathsAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
