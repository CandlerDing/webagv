...
private static final String[] allFormNames = {"cmd", "page", "idlist", "Id", "Code", "Name", "IdP", "Type", "Speed", "Commandcode", "StartTime", "Orderno", "Remark"};
private String[] DicKeys = {"Id", "Code", "Name", "IdP", "Type", "Speed", "Commandcode", "StartTime", "Orderno", "Remark"};
private String[][] DicValues = {{"int", "id", "-1", "hidden", ""}, {"string", "编码", "50", "text", ""}, {"string", "名称", "50", "text", ""}, {"int", "父ID", "-1", "hidden", ""}, {"string", "类型", "50", "text", ""}, {"double", "小车速度", "0", "text", ""}, {"string", "命令代码", "50", "text", ""}, {"string", "开始时间", "50", "text", ""}, {"int", "排序", "-1", "text", ""}, {"string", "备注", "50", "text", ""}};
private String KeyField = "Id";
private String OperateField = DicKeys[DicKeys.length-1];//操作字段
private String[] AllFields = {"Id", "Code", "Name", "IdP", "Type", "Speed", "Commandcode", "StartTime", "Orderno", "Remark"};
private String[] ListFields = {"Code", "Name", "Type", "Speed", "Commandcode", "StartTime", "Orderno", "Remark"};
private String[] PrintFields = {"Code", "Name", "Type", "Speed", "Commandcode", "StartTime", "Orderno", "Remark"};
private String[] FormFields = {"Code", "Name", "Type", "Speed", "Commandcode", "StartTime", "Orderno", "Remark"};
private String[] QueryFields = {"Name", "Type", "Speed", "Commandcode"};
private Commands getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    Commands v = new Commands();
    ...
    if (cmd.equals("update") || cmd.equals("jqueryUpdate") ) {
        ...
        v.paramSpeed(request, "Speed");
        v.paramCommandcode(request, "Commandcode");
        ...
        v.setSpeed(ParamUtils.getDoubleParameter(request, "Speed", 0));
        v.setCommandcode(ParamUtils.getParameter(request, "Commandcode", ""));
...
private List getListRows(javax.servlet.http.HttpServletRequest request, Commands pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        row.add("" + v.getSpeed());
        row.add(Tool.jsSpecialChars(v.getCommandcode()));
...
private void setListData(javax.servlet.http.HttpServletRequest request, Commands pv, List cdt)
{
    List rows = new ArrayList();
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, Commands pv, boolean isAll)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    ...
    qvali = ParamUtils.getIntParameter(request, "_Speed_", -1);
    QueryValues.add(""+qvali);
    if (qvali >= 0) {
        cdt.add("Speed=" + qvali);
    }
    qval = ParamUtils.getParameter(request, "_Commandcode_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("Commandcode like '%" + qval + "%'");
    }
    if (!isAll) {
...
private void setList(javax.servlet.http.HttpServletRequest request, boolean isAll)
{
    ...
    request.setAttribute("classname", "Commands");
    request.setAttribute("describe", "命令");
    request.setAttribute("OperateField",OperateField);
}
private List getListJsonRows(javax.servlet.http.HttpServletRequest request, Commands pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        map.put("Commandcode",Tool.jsSpecialChars(v.getCommandcode()));
...
private void setListJsonData(javax.servlet.http.HttpServletRequest request, Commands pv, List cdt)
{
...
private void setForm(javax.servlet.http.HttpServletRequest request, Commands form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    request.setAttribute("dickeys", DicKeys);
 ...
 */
private boolean isDuplicated(Commands form, String cmd)
{
    List cdt = new ArrayList();
...
private List mkRtn(String cmd, String forward, Commands form)
{
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    String cmd = ParamUtils.getParameter(request, "cmd", "list");
    log.debug("cmd=" + cmd);
    Commands form = getByParameterDb(request);
    if (cmd.equals("list"))
    {
        setList(request);
...
forwardMap.put("list", "CommandsAction.jsp");
forwardMap.put("failure", "CommandsForm.jsp");
forwardMap.put("success", "CommandsList.jsp");
forwardMap.put("listjquery", "CommandsListJquery.jsp");
if(userInfo!=null)
{
...
forwardMap.put("input", "CommandsForm.jsp");
forwardMap.put("jqueryInput", "CommandsFormJquery.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script type=\"text/JavaScript\">window.location='CommandsAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
