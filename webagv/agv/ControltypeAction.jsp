...
<%!
private static Log log = LogFactory.getLog(Controltype.class);
private static final String[] allFormNames = {"cmd", "page", "idlist", "Id", "Code", "Name", "IdP", "Retvalue", "Type", "Priority", "Orderno", "Remark"};
private String[] DicKeys = {"Id", "Code", "Name", "IdP", "Retvalue", "Type", "Priority", "Orderno", "Remark"};
private String[][] DicValues = {{"int", "ID", "-1", "hidden", ""}, {"string", "编码", "50", "text", ""}, {"string", "名称", "50", "text", ""}, {"int", "父ID", "-1", "hidden", ""}, {"string", "标识", "50", "text", ""}, {"string", "类型", "50", "text", ""}, {"int", "优先级", "-1", "text", ""}, {"int", "排序", "-1", "text", ""}, {"string", "备注", "50", "text", ""}};
private String KeyField = "Id";
private String OperateField = DicKeys[DicKeys.length-1];//操作字段
private String[] AllFields = {"Id", "Code", "Name", "IdP", "Retvalue", "Type", "Priority", "Orderno", "Remark"};
private String[] ListFields = {"Code", "Name", "Retvalue", "Type", "Priority", "Orderno", "Remark"};
private String[] PrintFields = {"Code", "Name", "Retvalue", "Type", "Priority", "Orderno", "Remark"};
private String[] FormFields = {"Code", "Name", "Retvalue", "Type", "Priority", "Orderno"};
private String[] QueryFields = {"Name", "Retvalue", "Type", "Priority"};
private Controltype getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("update") || cmd.equals("jqueryUpdate") ) {
        ...
        v.paramPriority(request, "Priority");
        ...
        v.setPriority(ParamUtils.getIntParameter(request, "Priority", 1));
...
private List getListRows(javax.servlet.http.HttpServletRequest request, Controltype pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        Controltype v = (Controltype)it.next();
...
private void setListData(javax.servlet.http.HttpServletRequest request, Controltype pv, List cdt)
{
    List rows = new ArrayList();
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, Controltype pv, boolean isAll)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    ...
    qvali = ParamUtils.getIntParameter(request, "_Priority_", -1);
    QueryValues.add(""+qvali);
    if (qvali >= 0) {
        cdt.add("Priority=" + qvali);
    }
    if (!isAll) {
...
{
    UserInfo userInfo = Tool.getUserInfo(request);
    Controltype pv = new Controltype();
    ...
    request.setAttribute("classname", "Controltype");
    request.setAttribute("describe", "控制方式");
    request.setAttribute("OperateField",OperateField);
}
private List getListJsonRows(javax.servlet.http.HttpServletRequest request, Controltype pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        map.put("Priority","" + v.getPriority());
...
private void setListJsonData(javax.servlet.http.HttpServletRequest request, Controltype pv, List cdt)
{
...
private void setForm(javax.servlet.http.HttpServletRequest request, Controltype form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    request.setAttribute("dickeys", DicKeys);
 ...
 */
private boolean isDuplicated(Controltype form, String cmd)
{
    List cdt = new ArrayList();
...
private List mkRtn(String cmd, String forward, Controltype form)
{
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    Controltype form = getByParameterDb(request);
    if (cmd.equals("list"))
    {
        setList(request);
    ...
    if (cmd.equals("create"))
    {
        ...
        Controltype ac = new Controltype();
         if(pid>0) //有父
        {
          Controltype formP = form.getById(pid);
          ...
          {           
            ...
            {
              form.setCode(cu.getNextCode(((Controltype)aclist.get(0)).getCode(),cu.getLevel(((Controltype)aclist.get(0)).getCode())));
            }
          }
        }else
        {         
          ...
          {
            form.setCode(cu.getNextCode(((Controltype)aclist.get(0)).getCode(),1));
          }
        }
...
forwardMap.put("list", "ControltypeAction.jsp");
forwardMap.put("failure", "ControltypeForm.jsp");
forwardMap.put("success", "ControltypeList.jsp");
forwardMap.put("listjquery", "ControltypeListJquery.jsp");
if(userInfo!=null)
{
...
forwardMap.put("input", "ControltypeForm.jsp");
forwardMap.put("jqueryInput", "ControltypeFormJquery.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script type=\"text/JavaScript\">window.location='ControltypeAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
