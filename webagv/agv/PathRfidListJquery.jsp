...
$(function () {
 ...
 function initData(){
    datagrid = $("#dadagrid").datagrid({
            url: 'PathRfidAction.jsp?cmd=json',
            iconCls: 'icon-save',
            pagination: true,
        	 rownumbers: true,
        ...
        queryParams : sy.serializeObject($("#PathRfid_queryForm").form()),
        	toolbar : '#toolbar',
...
function searchFun()
{
    	$("#dadagrid").datagrid("load",sy.serializeObject($("#PathRfid_queryForm").form()));
}
//点击清空按钮出发事件
function clearSearch() {
    	$("#dadagrid").datagrid("load", {});//重新加载数据，无填写数据，向后台传递值则为空
    	$("#PathRfid_queryForm").find("input").val("");//找到form表单下的所有input标签并清空
}
//添加
function addFun() {
...
<form id="PathRfid_queryForm" name="PathRfid_queryForm" method="post">
