...
<%!
private static Log log = LogFactory.getLog(TaskType.class);
...
private TaskType getByParameterDb(javax.servlet.http.HttpServletRequest request)
{ 
    UserInfo userInfo = Tool.getUserInfo(request);
    TaskType v = new TaskType();
...
private List getListRows(javax.servlet.http.HttpServletRequest request, TaskType pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
...
private void setListData(javax.servlet.http.HttpServletRequest request, TaskType pv, List cdt)
{
    List rows = new ArrayList();
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, TaskType pv, boolean isAll)
{
    UserInfo userInfo = Tool.getUserInfo(request);
...
private void setList(javax.servlet.http.HttpServletRequest request, boolean isAll)
{
    ...
    request.setAttribute("classname", "TaskType");
    request.setAttribute("describe", "任务类型");
    request.setAttribute("OperateField",OperateField);
}
private List getListJsonRows(javax.servlet.http.HttpServletRequest request, TaskType pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
...
private void setListJsonData(javax.servlet.http.HttpServletRequest request, TaskType pv, List cdt)
{
...
private void setForm(javax.servlet.http.HttpServletRequest request, TaskType form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    request.setAttribute("dickeys", DicKeys);
 ...
 */
private boolean isDuplicated(TaskType form, String cmd)
{
    List cdt = new ArrayList();
...
private List mkRtn(String cmd, String forward, TaskType form)
{
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    String cmd = ParamUtils.getParameter(request, "cmd", "list");
    log.debug("cmd=" + cmd);
    TaskType form = getByParameterDb(request);
    if (cmd.equals("list"))
    {
        setList(request);
...
forwardMap.put("list", "TaskTypeAction.jsp");
forwardMap.put("failure", "TaskTypeForm.jsp");
forwardMap.put("success", "TaskTypeList.jsp");
forwardMap.put("listjquery", "TaskTypeListJquery.jsp");
if(userInfo!=null)
{
...
forwardMap.put("input", "TaskTypeForm.jsp");
forwardMap.put("jqueryInput", "TaskTypeFormJquery.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script type=\"text/JavaScript\">window.location='TaskTypeAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
