...
<%!
private static Log log = LogFactory.getLog(PathBs.class);
private static final String[] allFormNames = {"cmd", "page", "idlist", "Id", "Code", "Name", "IdP", "OnlyId", "Orderno", "Remark"};
private String[] DicKeys = {"Id", "Code", "Name", "IdP", "OnlyId", "Orderno", "Remark"};
private String[][] DicValues = {{"int", "id", "-1", "hidden", ""}, {"string", "编码", "50", "text", ""}, {"string", "名称", "50", "text", ""}, {"int", "父ID", "-1", "hidden", ""}, {"string", "标识ID", "50", "text", ""}, {"int", "排序", "-1", "text", ""}, {"string", "备注", "50", "text", ""}};
private String KeyField = "Id";
private String OperateField = DicKeys[DicKeys.length-1];//操作字段
private String[] AllFields = {"Id", "Code", "Name", "IdP", "OnlyId", "Orderno", "Remark"};
private String[] ListFields = {"Code", "Name", "OnlyId", "Orderno", "Remark"};
private String[] PrintFields = {"Code", "Name", "OnlyId", "Orderno", "Remark"};
private String[] FormFields = {"Code", "Name", "OnlyId", "Orderno", "Remark"};
private String[] QueryFields = {"Name", "OnlyId"};
private PathBs getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("update") || cmd.equals("jqueryUpdate") ) {
        ...
        v.setOnlyId(ParamUtils.getParameter(request, "OnlyId", ""));
...
private List getListRows(javax.servlet.http.HttpServletRequest request, PathBs pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
...
private void setListData(javax.servlet.http.HttpServletRequest request, PathBs pv, List cdt)
{
    List rows = new ArrayList();
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, PathBs pv, boolean isAll)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    ...
    qval = ParamUtils.getParameter(request, "_OnlyId_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("OnlyId like '%" + qval + "%'");
    }
    if (!isAll) {
...
private void setList(javax.servlet.http.HttpServletRequest request, boolean isAll)
{
    ...
    request.setAttribute("classname", "PathBs");
    request.setAttribute("describe", "路径标识点管理");
    request.setAttribute("OperateField",OperateField);
}
private List getListJsonRows(javax.servlet.http.HttpServletRequest request, PathBs pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        map.put("OnlyId",Tool.jsSpecialChars(v.getOnlyId()));
...
private void setListJsonData(javax.servlet.http.HttpServletRequest request, PathBs pv, List cdt)
{
...
private void setForm(javax.servlet.http.HttpServletRequest request, PathBs form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    request.setAttribute("dickeys", DicKeys);
 ...
 */
private boolean isDuplicated(PathBs form, String cmd)
{
    List cdt = new ArrayList();
...
private List mkRtn(String cmd, String forward, PathBs form)
{
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    String cmd = ParamUtils.getParameter(request, "cmd", "list");
    log.debug("cmd=" + cmd);
    PathBs form = getByParameterDb(request);
    if (cmd.equals("list"))
    {
        setList(request);
...
forwardMap.put("list", "PathBsAction.jsp");
forwardMap.put("failure", "PathBsForm.jsp");
forwardMap.put("success", "PathBsList.jsp");
forwardMap.put("listjquery", "PathBsListJquery.jsp");
if(userInfo!=null)
{
...
forwardMap.put("input", "PathBsForm.jsp");
forwardMap.put("jqueryInput", "PathBsFormJquery.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script type=\"text/JavaScript\">window.location='PathBsAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
