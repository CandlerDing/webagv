...
<%!
private static Log log = LogFactory.getLog(TaskState.class);
private static final String[] allFormNames = {"cmd", "page", "idlist", "Id", "Code", "Name", "Orderno", "Remark"};
private String[] DicKeys = {"Id", "Code", "Name", "Orderno", "Remark"};
private String[][] DicValues = {{"int", "id", "-1", "hidden", ""}, {"string", "编码", "50", "text", ""}, {"string", "名称", "50", "text", ""}, {"int", "排序", "-1", "text", ""}, {"string", "备注", "50", "text", ""}};
private String KeyField = "Id"; 
private String OperateField = DicKeys[DicKeys.length-1];//操作字段
private String[] AllFields = {"Id", "Code", "Name", "Orderno", "Remark"};
...
private TaskState getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    TaskState v = new TaskState();
...
private List getListRows(javax.servlet.http.HttpServletRequest request, TaskState pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
...
private void setListData(javax.servlet.http.HttpServletRequest request, TaskState pv, List cdt)
{
    List rows = new ArrayList();
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, TaskState pv, boolean isAll)
{
    UserInfo userInfo = Tool.getUserInfo(request);
...
private void setList(javax.servlet.http.HttpServletRequest request, boolean isAll)
{
    ...
    request.setAttribute("classname", "TaskState");
    request.setAttribute("describe", "任务状态");
    request.setAttribute("OperateField",OperateField);
}
private List getListJsonRows(javax.servlet.http.HttpServletRequest request, TaskState pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
...
private void setListJsonData(javax.servlet.http.HttpServletRequest request, TaskState pv, List cdt)
{
...
private void setForm(javax.servlet.http.HttpServletRequest request, TaskState form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    request.setAttribute("dickeys", DicKeys);
 ...
 */
private boolean isDuplicated(TaskState form, String cmd)
{
    List cdt = new ArrayList();
...
private List mkRtn(String cmd, String forward, TaskState form)
{
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    String cmd = ParamUtils.getParameter(request, "cmd", "list");
    log.debug("cmd=" + cmd);
    TaskState form = getByParameterDb(request);
    if (cmd.equals("list"))
    {
        setList(request);
...
forwardMap.put("list", "TaskStateAction.jsp");
forwardMap.put("failure", "TaskStateForm.jsp");
forwardMap.put("success", "TaskStateList.jsp");
forwardMap.put("listjquery", "TaskStateListJquery.jsp");
if(userInfo!=null)
{
...
forwardMap.put("input", "TaskStateForm.jsp");
forwardMap.put("jqueryInput", "TaskStateFormJquery.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script type=\"text/JavaScript\">window.location='TaskStateAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
