...
$(function () {
 ...
 function initData(){
    datagrid = $("#dadagrid").datagrid({
        ...
        queryParams : sy.serializeObject($("#Paths_queryForm").form()),
        	toolbar : '#toolbar',
...
function searchFun()
{
    	$("#dadagrid").datagrid("load",sy.serializeObject($("#Paths_queryForm").form()));
}
//点击清空按钮出发事件
function clearSearch() {
    	$("#dadagrid").datagrid("load", {});//重新加载数据，无填写数据，向后台传递值则为空
    	$("#Paths_queryForm").find("input").val("");//找到form表单下的所有input标签并清空
}
//添加
function addFun() {
...
<form id="Paths_queryForm" name="Paths_queryForm" method="post">
