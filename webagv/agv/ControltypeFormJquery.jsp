...
function save() {
    	$("#postForm_Controltype").attr("target","self");
    	$("#postForm_Controltype").on("submit", function(event) {
        	$.messager.progress({
            		title : "提示",
            		text : "请稍后...."
        	});
    	});
    	$("#postForm_Controltype").form({
    	...
    	var isValid = $("#postForm_Controltype").form("validate");
    	if (!isValid){
...
<input name="Priority"  id="Priority" class="easyui-numberbox" size="-1" value="<%=v.getPriority()%>"">
</td></tr>
...
<input type="hidden" name="Remark" id="Remark" value="<%=v.getRemark()%>">
</table>
</form>
</div>
</div>
