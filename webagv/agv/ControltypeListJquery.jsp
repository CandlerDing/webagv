...
$(function () {
 ...
 function initData(){
    datagrid = $("#dadagrid").datagrid({
            url: 'ControltypeAction.jsp?cmd=json',
            iconCls: 'icon-save',
            pagination: true,
        	 rownumbers: true,
        ...
        queryParams : sy.serializeObject($("#Controltype_queryForm").form()),
        	toolbar : '#toolbar',
...
function searchFun()
{
    	$("#dadagrid").datagrid("load",sy.serializeObject($("#Controltype_queryForm").form()));
}
//点击清空按钮出发事件
function clearSearch() {
    	$("#dadagrid").datagrid("load", {});//重新加载数据，无填写数据，向后台传递值则为空
    	$("#Controltype_queryForm").find("input").val("");//找到form表单下的所有input标签并清空
}
//添加
function addFun() {
...
<form id="Controltype_queryForm" name="Controltype_queryForm" method="post">
